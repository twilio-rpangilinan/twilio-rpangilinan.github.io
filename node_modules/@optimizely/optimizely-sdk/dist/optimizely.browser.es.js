...
import { LocalStoragePendingEventsDispatcher, LogTierV1EventProcessor } from '@optimizely/js-sdk-event-processor';
import { NOTIFICATION_TYPES as NOTIFICATION_TYPES$1, sprintf, generateUUID, keyBy as keyBy$1, objectValues } from '@optimizely/js-sdk-utils';
...
var NOTIFICATION_TYPES = NOTIFICATION_TYPES$1;
...
var getAttributeId = function (projectConfig, attributeKey, logger) {
    ...
    if (attribute) {
        if (hasReservedPrefix) {
            logger.log(LOG_LEVEL.WARNING, sprintf('Attribute %s unexpectedly has reserved prefix %s; using attribute ID instead of reserved attribute name.', attributeKey, RESERVED_ATTRIBUTE_PREFIX));
        }
        return attribute.id;
    }
    ...
    logger.log(LOG_LEVEL.DEBUG, sprintf(ERROR_MESSAGES.UNRECOGNIZED_ATTRIBUTE, MODULE_NAME$1, attributeKey));
    return null;
};
/**
...
var getExperimentFromId = function (projectConfig, experimentId, logger) {
    ...
    logger.log(LOG_LEVEL.ERROR, sprintf(ERROR_MESSAGES.INVALID_EXPERIMENT_ID, MODULE_NAME$1, experimentId));
    return null;
};
/**
...
var getFeatureFromKey = function (projectConfig, featureKey, logger) {
    ...
    logger.log(LOG_LEVEL.ERROR, sprintf(ERROR_MESSAGES.FEATURE_NOT_IN_DATAFILE, MODULE_NAME$1, featureKey));
    return null;
};
/**
...
var getVariableForFeature = function (projectConfig, featureKey, variableKey, logger) {
    ...
    if (!variable) {
        logger.log(LOG_LEVEL.ERROR, sprintf(ERROR_MESSAGES.VARIABLE_KEY_NOT_IN_DATAFILE, MODULE_NAME$1, variableKey, featureKey));
        return null;
    }
    return variable;
};
...
var getVariableValueForVariation = function (projectConfig, variable, variation, logger) {
    ...
    if (!projectConfig.variationVariableUsageMap.hasOwnProperty(variation.id)) {
        logger.log(LOG_LEVEL.ERROR, sprintf(ERROR_MESSAGES.VARIATION_ID_NOT_IN_DATAFILE_NO_EXPERIMENT, MODULE_NAME$1, variation.id));
        return null;
    }
    ...
    switch (variableType) {
        case FEATURE_VARIABLE_TYPES.BOOLEAN:
            if (variableValue !== 'true' && variableValue !== 'false') {
                logger.log(LOG_LEVEL.ERROR, sprintf(ERROR_MESSAGES.UNABLE_TO_CAST_VALUE, MODULE_NAME$1, variableValue, variableType));
                castValue = null;
            }
            else {
...
var tryCreatingProjectConfig = function (config) {
    ...
    if (config.jsonSchemaValidator) {
        try {
            ...
            config.logger.log(LOG_LEVEL.INFO, sprintf(LOG_MESSAGES.VALID_DATAFILE, MODULE_NAME$1));
        }
        catch (error) {
            return { configObj: null, error: error };
        }
        ...
        config.logger.log(LOG_LEVEL.INFO, sprintf(LOG_MESSAGES.SKIPPING_JSON_VALIDATION, MODULE_NAME$1));
    }
        ...
        try {
            (this.notificationListeners[notificationType] || []).forEach(function (listenerEntry) {
                ...
                try {
                    ...
                    _this.logger.log(LOG_LEVEL.ERROR, sprintf(LOG_MESSAGES.NOTIFICATION_LISTENER_EXCEPTION, MODULE_NAME$3, notificationType, ex.message));
                }
            });
        }
        catch (e) {
...
var bucket = function (bucketerParams) {
    ...
    if (groupId) {
        ...
        if (!group) {
            throw new Error(sprintf(ERROR_MESSAGES.INVALID_GROUP_ID, MODULE_NAME$4, groupId));
        }
        if (group.policy === RANDOM_POLICY) {
            ...
            if (bucketedExperimentId === null) {
                var notbucketedInAnyExperimentLogMessage = sprintf(LOG_MESSAGES.USER_NOT_IN_ANY_EXPERIMENT, MODULE_NAME$4, bucketerParams.userId, groupId);
            ...
            if (bucketedExperimentId !== bucketerParams.experimentId) {
                var notBucketedIntoExperimentOfGroupLogMessage = sprintf(LOG_MESSAGES.USER_NOT_BUCKETED_INTO_EXPERIMENT_IN_GROUP, MODULE_NAME$4, bucketerParams.userId, bucketerParams.experimentKey, groupId);
            ...
            var bucketedIntoExperimentOfGroupLogMessage = sprintf(LOG_MESSAGES.USER_BUCKETED_INTO_EXPERIMENT_IN_GROUP, MODULE_NAME$4, bucketerParams.userId, bucketerParams.experimentKey, groupId);
    ...
    var bucketedUserLogMessage = sprintf(LOG_MESSAGES.USER_ASSIGNED_TO_EXPERIMENT_BUCKET, MODULE_NAME$4, bucketValue, bucketerParams.userId);
    ...
    if (entityId !== null) {
        if (!bucketerParams.variationIdMap[entityId]) {
            if (entityId) {
                var invalidVariationIdLogMessage = sprintf(LOG_MESSAGES.INVALID_VARIATION_ID, MODULE_NAME$4);
...
var bucketUserIntoExperiment = function (group, bucketingId, userId, logger) {
    ...
    logger.log(LOG_LEVEL.DEBUG, sprintf(LOG_MESSAGES.USER_ASSIGNED_TO_EXPERIMENT_BUCKET, MODULE_NAME$4, bucketValue, userId));
...
var _generateBucketValue = function (bucketingKey) {
    try {
        ...
        throw new Error(sprintf(ERROR_MESSAGES.INVALID_BUCKETING_ID, MODULE_NAME$4, bucketingKey, ex.message));
    }
};
...
function splitVersion(version) {
    ...
    if (hasWhiteSpaces(version)) {
        logger$1.warn(LOG_MESSAGES.UNKNOWN_MATCH_TYPE, MODULE_NAME$5, version);
        return null;
    }
...
function evaluate$1(condition, userAttributes) {
    ...
    if (typeof conditionMatch !== 'undefined' && MATCH_TYPES.indexOf(conditionMatch) === -1) {
        logger$2.warn(LOG_MESSAGES.UNKNOWN_MATCH_TYPE, MODULE_NAME$6, JSON.stringify(condition));
        return null;
    }
    var attributeKey = condition.name;
    if (!userAttributes.hasOwnProperty(attributeKey) && conditionMatch != EXISTS_MATCH_TYPE) {
        logger$2.debug(LOG_MESSAGES.MISSING_ATTRIBUTE_VALUE, MODULE_NAME$6, JSON.stringify(condition), attributeKey);
        return null;
    }
...
function exactEvaluator(condition, userAttributes) {
    ...
    if (!isValueTypeValidForExactConditions(conditionValue) ||
        ...
        logger$2.warn(LOG_MESSAGES.UNEXPECTED_CONDITION_VALUE, MODULE_NAME$6, JSON.stringify(condition));
        return null;
    }
    if (userValue === null) {
        logger$2.debug(LOG_MESSAGES.UNEXPECTED_TYPE_NULL, MODULE_NAME$6, JSON.stringify(condition), conditionName);
        return null;
    }
    if (!isValueTypeValidForExactConditions(userValue) || conditionValueType !== userValueType) {
        logger$2.warn(LOG_MESSAGES.UNEXPECTED_TYPE, MODULE_NAME$6, JSON.stringify(condition), userValueType, conditionName);
        return null;
    }
    if (fns.isNumber(userValue) && !fns.isSafeInteger(userValue)) {
        logger$2.warn(LOG_MESSAGES.OUT_OF_BOUNDS, MODULE_NAME$6, JSON.stringify(condition), conditionName);
        return null;
    }
...
var AudienceEvaluator = /** @class */ (function () {
    ...
    AudienceEvaluator.prototype.evaluate = function (audienceConditions, audiencesById, userAttributes) {
        ...
        var evaluateAudience = function (audienceId) {
            ...
            if (audience) {
                logger$3.log(LOG_LEVEL.DEBUG, sprintf(LOG_MESSAGES.EVALUATING_AUDIENCE, MODULE_NAME$7, audienceId, JSON.stringify(audience.conditions)));
                ...
                logger$3.log(LOG_LEVEL.DEBUG, sprintf(LOG_MESSAGES.AUDIENCE_EVALUATION_RESULT, MODULE_NAME$7, audienceId, resultText));
                return result;
            }
            return null;
        };
    ...
    AudienceEvaluator.prototype.evaluateConditionWithUserAttributes = function (userAttributes, condition) {
        ...
        if (!evaluator) {
            logger$3.log(LOG_LEVEL.WARNING, sprintf(LOG_MESSAGES.UNKNOWN_CONDITION_TYPE, MODULE_NAME$7, JSON.stringify(condition)));
            return null;
        }
        try {
            ...
            logger$3.log(LOG_LEVEL.ERROR, sprintf(ERROR_MESSAGES.CONDITION_EVALUATOR_ERROR, MODULE_NAME$7, condition.type, err.message));
        }
        return null;
    };
...
var DecisionService = /** @class */ (function () {
    ...
    DecisionService.prototype.getVariation = function (configObj, experiment, userId, attributes, options) {
        ...
        if (!this.checkIfExperimentIsActive(configObj, experimentKey)) {
            var experimentNotRunningLogMessage = sprintf(LOG_MESSAGES.EXPERIMENT_NOT_RUNNING, MODULE_NAME$8, experimentKey);
        ...
        if (!shouldIgnoreUPS) {
            ...
            if (variation) {
                var returningStoredVariationMessage = sprintf(LOG_MESSAGES.RETURNING_STORED_VARIATION, MODULE_NAME$8, variation.key, experimentKey, userId);
        ...
        if (!decisionifUserIsInAudience.result) {
            var userDoesNotMeetConditionsLogMessage = sprintf(LOG_MESSAGES.USER_NOT_IN_EXPERIMENT, MODULE_NAME$8, userId, experimentKey);
        ...
        if (!variation) {
            var userHasNoVariationLogMessage = sprintf(LOG_MESSAGES.USER_HAS_NO_VARIATION, MODULE_NAME$8, userId, experimentKey);
        ...
        var userInVariationLogMessage = sprintf(LOG_MESSAGES.USER_HAS_VARIATION, MODULE_NAME$8, userId, variation.key, experimentKey);
    ...
    DecisionService.prototype.getWhitelistedVariation = function (experiment, userId) {
        ...
        if (experiment.forcedVariations && experiment.forcedVariations.hasOwnProperty(userId)) {
            ...
            if (experiment.variationKeyMap.hasOwnProperty(forcedVariationKey)) {
                var forcedBucketingSucceededMessageLog = sprintf(LOG_MESSAGES.USER_FORCED_IN_VARIATION, MODULE_NAME$8, userId, forcedVariationKey);
                ...
                var forcedBucketingFailedMessageLog = sprintf(LOG_MESSAGES.FORCED_BUCKETING_FAILED, MODULE_NAME$8, forcedVariationKey, userId);
    ...
    DecisionService.prototype.checkIfUserIsInAudience = function (configObj, experiment, evaluationAttribute, attributes, loggingKey) {
        ...
        var evaluatingAudiencesCombinedMessage = sprintf(LOG_MESSAGES.EVALUATING_AUDIENCES_COMBINED, MODULE_NAME$8, evaluationAttribute, loggingKey || experiment.key, JSON.stringify(experimentAudienceConditions));
        ...
        var audienceEvaluationResultCombinedMessage = sprintf(LOG_MESSAGES.AUDIENCE_EVALUATION_RESULT_COMBINED, MODULE_NAME$8, evaluationAttribute, loggingKey || experiment.key, result.toString().toUpperCase());
    ...
    DecisionService.prototype.getStoredVariation = function (configObj, experiment, userId, experimentBucketMap) {
        if (experimentBucketMap.hasOwnProperty(experiment.id)) {
            ...
            if (configObj.variationIdMap.hasOwnProperty(variationId)) {
                ...
                this.logger.log(LOG_LEVEL.INFO, sprintf(LOG_MESSAGES.SAVED_VARIATION_NOT_FOUND, MODULE_NAME$8, userId, variationId, experiment.key));
            }
        }
        return null;
    };
    ...
    DecisionService.prototype.getUserProfile = function (userId) {
        ...
        try {
            ...
            this.logger.log(LOG_LEVEL.ERROR, sprintf(ERROR_MESSAGES.USER_PROFILE_LOOKUP_ERROR, MODULE_NAME$8, userId, ex.message));
        }
        return null;
    };
    /**
    ...
    DecisionService.prototype.saveUserProfile = function (experiment, variation, userId, experimentBucketMap) {
        ...
        try {
            ...
            this.logger.log(LOG_LEVEL.INFO, sprintf(LOG_MESSAGES.SAVED_VARIATION, MODULE_NAME$8, variation.key, experiment.key, userId));
        }
        catch (ex) {
            this.logger.log(LOG_LEVEL.ERROR, sprintf(ERROR_MESSAGES.USER_PROFILE_SAVE_ERROR, MODULE_NAME$8, userId, ex.message));
        }
    };
    /**
    ...
    DecisionService.prototype.getVariationForFeature = function (configObj, feature, userId, attributes, options) {
        ...
        if (rolloutDecision.variation) {
            var userInRolloutMessage = sprintf(LOG_MESSAGES.USER_IN_ROLLOUT, MODULE_NAME$8, userId, feature.key);
        ...
        var userNotInRolloutMessage = sprintf(LOG_MESSAGES.USER_NOT_IN_ROLLOUT, MODULE_NAME$8, userId, feature.key);
    ...
    DecisionService.prototype.getVariationForFeatureExperiment = function (configObj, feature, userId, attributes, options) {
        ...
        if (feature.experimentIds.length > 0) {
            ...
            var featureHasNoExperimentsMessage = sprintf(LOG_MESSAGES.FEATURE_HAS_NO_EXPERIMENTS, MODULE_NAME$8, feature.key);
    ...
    DecisionService.prototype.getVariationForRollout = function (configObj, feature, userId, attributes) {
        ...
        if (!feature.rolloutId) {
            var noRolloutExistsMessage = sprintf(LOG_MESSAGES.NO_ROLLOUT_EXISTS, MODULE_NAME$8, feature.key);
        ...
        if (!rollout) {
            var invalidRolloutIdMessage = sprintf(ERROR_MESSAGES.INVALID_ROLLOUT_ID, MODULE_NAME$8, feature.rolloutId, feature.key);
        ...
        if (rollout.experiments.length === 0) {
            var rolloutHasNoExperimentsMessage = sprintf(LOG_MESSAGES.ROLLOUT_HAS_NO_EXPERIMENTS, MODULE_NAME$8, feature.rolloutId);
        ...
        for (var index = 0; index < endIndex; index++) {
            ...
            decideReasons.push.apply(decideReasons, decisionifUserIsInAudience.reasons);
            if (!decisionifUserIsInAudience.result) {
                var userDoesNotMeetConditionsForTargetingRuleMessage = sprintf(LOG_MESSAGES.USER_DOESNT_MEET_CONDITIONS_FOR_TARGETING_RULE, MODULE_NAME$8, userId, loggingKey);
            ...
            var userMeetsConditionsForTargetingRuleMessage = sprintf(LOG_MESSAGES.USER_MEETS_CONDITIONS_FOR_TARGETING_RULE, MODULE_NAME$8, userId, loggingKey);
            ...
            if (variation) {
                var userBucketeredIntoTargetingRuleMessage = sprintf(LOG_MESSAGES.USER_BUCKETED_INTO_TARGETING_RULE, MODULE_NAME$8, userId, loggingKey);
                ...
                var userNotBucketeredIntoTargetingRuleMessage = sprintf(LOG_MESSAGES.USER_NOT_BUCKETED_INTO_TARGETING_RULE, MODULE_NAME$8, userId, loggingKey);
        ...
        if (decisionifUserIsInEveryoneRule.result) {
            var userMeetsConditionsForEveryoneTargetingRuleMessage = sprintf(LOG_MESSAGES.USER_MEETS_CONDITIONS_FOR_TARGETING_RULE, MODULE_NAME$8, userId, 'Everyone Else');
            ...
            if (variation) {
                var userBucketeredIntoEveryoneTargetingRuleMessage = sprintf(LOG_MESSAGES.USER_BUCKETED_INTO_EVERYONE_TARGETING_RULE, MODULE_NAME$8, userId);
                ...
                var userNotBucketeredIntoEveryoneTargetingRuleMessage = sprintf(LOG_MESSAGES.USER_NOT_BUCKETED_INTO_EVERYONE_TARGETING_RULE, MODULE_NAME$8, userId);
    ...
    DecisionService.prototype.getBucketingId = function (userId, attributes) {
        ...
        if (attributes != null &&
            ...
            if (typeof attributes[CONTROL_ATTRIBUTES.BUCKETING_ID] === 'string') {
                ...
                this.logger.log(LOG_LEVEL.DEBUG, sprintf(LOG_MESSAGES.VALID_BUCKETING_ID, MODULE_NAME$8, bucketingId));
            }
            else {
                this.logger.log(LOG_LEVEL.WARNING, sprintf(LOG_MESSAGES.BUCKETING_ID_NOT_STRING, MODULE_NAME$8));
            }
        }
        return bucketingId;
    };
    ...
    DecisionService.prototype.removeForcedVariation = function (userId, experimentId, experimentKey) {
        ...
        if (this.forcedVariationMap.hasOwnProperty(userId)) {
            ...
            this.logger.log(LOG_LEVEL.DEBUG, sprintf(LOG_MESSAGES.VARIATION_REMOVED_FOR_USER, MODULE_NAME$8, experimentKey, userId));
        }
        else {
    ...
    DecisionService.prototype.setInForcedVariationMap = function (userId, experimentId, variationId) {
        ...
        this.logger.log(LOG_LEVEL.DEBUG, sprintf(LOG_MESSAGES.USER_MAPPED_TO_FORCED_VARIATION, MODULE_NAME$8, variationId, experimentId, userId));
    };
    /**
    ...
    DecisionService.prototype.getForcedVariation = function (configObj, experimentKey, userId) {
        ...
        if (!experimentToVariationMap) {
            this.logger.log(LOG_LEVEL.DEBUG, sprintf(LOG_MESSAGES.USER_HAS_NO_FORCED_VARIATION, MODULE_NAME$8, userId));
            return {
                result: null,
                reasons: decideReasons,
            };
        ...
        try {
            ...
            if (experiment.hasOwnProperty('id')) {
                ...
                var improperlyFormattedExperimentMessage = sprintf(ERROR_MESSAGES.IMPROPERLY_FORMATTED_EXPERIMENT, MODULE_NAME$8, experimentKey);
        ...
        if (!variationId) {
            this.logger.log(LOG_LEVEL.DEBUG, sprintf(LOG_MESSAGES.USER_HAS_NO_FORCED_VARIATION_FOR_EXPERIMENT, MODULE_NAME$8, experimentKey, userId));
            return {
                result: null,
                reasons: decideReasons,
            };
        ...
        if (variationKey) {
            var userHasForcedVariationMessage = sprintf(LOG_MESSAGES.USER_HAS_FORCED_VARIATION, MODULE_NAME$8, variationKey, experimentKey, userId);
    ...
    DecisionService.prototype.setForcedVariation = function (configObj, experimentKey, userId, variationKey) {
        if (variationKey != null && !validate$1(variationKey)) {
            this.logger.log(LOG_LEVEL.ERROR, sprintf(ERROR_MESSAGES.INVALID_VARIATION_KEY, MODULE_NAME$8));
            return false;
        }
        var experimentId;
        try {
            ...
            if (experiment.hasOwnProperty('id')) {
                ...
                // catching improperly formatted experiments
                this.logger.log(LOG_LEVEL.ERROR, sprintf(ERROR_MESSAGES.IMPROPERLY_FORMATTED_EXPERIMENT, MODULE_NAME$8, experimentKey));
                return false;
            }
        }
        catch (ex) {
        ...
        if (!variationId) {
            this.logger.log(LOG_LEVEL.ERROR, sprintf(ERROR_MESSAGES.NO_VARIATION_FOR_EXPERIMENT_KEY, MODULE_NAME$8, variationKey, experimentKey));
            return false;
        }
        try {
    ...
    if (eventTags.hasOwnProperty(REVENUE_EVENT_METRIC_NAME)) {
        ...
        if (typeof rawValue === 'string') {
            ...
            if (isNaN(parsedRevenueValue)) {
                logger.log(LOG_LEVEL.INFO, sprintf(LOG_MESSAGES.FAILED_TO_PARSE_REVENUE, MODULE_NAME$9, rawValue));
                return null;
            }
            logger.log(LOG_LEVEL.INFO, sprintf(LOG_MESSAGES.PARSED_REVENUE_VALUE, MODULE_NAME$9, parsedRevenueValue));
    ...
    if (eventTags.hasOwnProperty(VALUE_EVENT_METRIC_NAME)) {
        ...
        if (typeof rawValue === 'string') {
            ...
            if (isNaN(parsedEventValue)) {
                logger.log(LOG_LEVEL.INFO, sprintf(LOG_MESSAGES.FAILED_TO_PARSE_VALUE, MODULE_NAME$9, rawValue));
                return null;
            }
            logger.log(LOG_LEVEL.INFO, sprintf(LOG_MESSAGES.PARSED_NUMERIC_VALUE, MODULE_NAME$9, parsedEventValue));
 ...
 * limitations under the License.
 */
var logger$4 = getLogger('EVENT_BUILDER');
/**
...
function createEventProcessor() {
    ...
    return new (LogTierV1EventProcessor.bind.apply(LogTierV1EventProcessor, __spreadArrays([void 0], args)))();
}
...
var Optimizely = /** @class */ (function () {
    function Optimizely(config) {
        ...
        if (VALID_CLIENT_ENGINES.indexOf(clientEngine) === -1) {
            config.logger.log(LOG_LEVEL.INFO, sprintf(LOG_MESSAGES.INVALID_CLIENT_ENGINE, MODULE_NAME$d, clientEngine));
        ...
        if (!Array.isArray(decideOptionsArray)) {
            this.logger.log(LOG_LEVEL.DEBUG, sprintf(LOG_MESSAGES.INVALID_DEFAULT_DECIDE_OPTIONS, MODULE_NAME$d));
            decideOptionsArray = [];
        }
        ...
        decideOptionsArray.forEach(function (option) {
            ...
            if (OptimizelyDecideOption[option]) {
                ...
                _this.logger.log(LOG_LEVEL.WARNING, sprintf(LOG_MESSAGES.UNRECOGNIZED_DECIDE_OPTION, MODULE_NAME$d, option));
            }
        });
        ...
        this.disposeOnUpdate = this.projectConfigManager.onUpdate(function (configObj) {
            _this.logger.log(LOG_LEVEL.INFO, sprintf(LOG_MESSAGES.UPDATED_OPTIMIZELY_CONFIG, MODULE_NAME$d, configObj.revision, configObj.projectId));
        ...
        if (config.userProfileService) {
            try {
                if (validate$4(config.userProfileService)) {
                    ...
                    this.logger.log(LOG_LEVEL.INFO, sprintf(LOG_MESSAGES.VALID_USER_PROFILE_SERVICE, MODULE_NAME$d));
                }
            }
            catch (ex) {
    ...
    Optimizely.prototype.activate = function (experimentKey, userId, attributes) {
        try {
            if (!this.isValidInstance()) {
                this.logger.log(LOG_LEVEL.ERROR, sprintf(LOG_MESSAGES.INVALID_OBJECT, MODULE_NAME$d, 'activate'));
                return null;
            }
            ...
            try {
                ...
                if (!isRunning(configObj, experimentKey)) {
                    var shouldNotDispatchActivateLogMessage = sprintf(LOG_MESSAGES.SHOULD_NOT_DISPATCH_ACTIVATE, MODULE_NAME$d, experimentKey);
                ...
                this.logger.log(LOG_LEVEL.ERROR, ex.message);
                var failedActivationLogMessage = sprintf(LOG_MESSAGES.NOT_ACTIVATING_USER, MODULE_NAME$d, userId, experimentKey);
    ...
    Optimizely.prototype.track = function (eventKey, userId, attributes, eventTags) {
        try {
            if (!this.isValidInstance()) {
                this.logger.log(LOG_LEVEL.ERROR, sprintf(LOG_MESSAGES.INVALID_OBJECT, MODULE_NAME$d, 'track'));
                return;
            }
            ...
            if (!eventWithKeyExists(configObj, eventKey)) {
                this.logger.log(LOG_LEVEL.WARNING, sprintf(LOG_MESSAGES.EVENT_KEY_NOT_FOUND, MODULE_NAME$d, eventKey));
                this.logger.log(LOG_LEVEL.WARNING, sprintf(LOG_MESSAGES.NOT_TRACKING_USER, MODULE_NAME$d, userId));
                return;
            }
            ...
            this.logger.log(LOG_LEVEL.INFO, sprintf(LOG_MESSAGES.TRACK_EVENT, MODULE_NAME$d, eventKey, userId));
            // TODO is it okay to not pass a projectConfig as second argument
            ...
            this.logger.log(LOG_LEVEL.ERROR, e.message);
            this.errorHandler.handleError(e);
            var failedTrackLogMessage = sprintf(LOG_MESSAGES.NOT_TRACKING_USER, MODULE_NAME$d, userId);
    ...
    Optimizely.prototype.getVariation = function (experimentKey, userId, attributes) {
        try {
            if (!this.isValidInstance()) {
                this.logger.log(LOG_LEVEL.ERROR, sprintf(LOG_MESSAGES.INVALID_OBJECT, MODULE_NAME$d, 'getVariation'));
                return null;
            }
            try {
                if (!this.validateInputs({ experiment_key: experimentKey, user_id: userId }, attributes)) {
                ...
                if (!experiment) {
                    this.logger.log(LOG_LEVEL.DEBUG, sprintf(ERROR_MESSAGES.INVALID_EXPERIMENT_KEY, MODULE_NAME$d, experimentKey));
                    return null;
                }
        ...
        try {
            if (stringInputs.hasOwnProperty('user_id')) {
                ...
                if (typeof userId !== 'string' || userId === null || userId === 'undefined') {
                    throw new Error(sprintf(ERROR_MESSAGES.INVALID_INPUT_FORMAT, MODULE_NAME$d, 'user_id'));
                }
    ...
    Optimizely.prototype.isFeatureEnabled = function (featureKey, userId, attributes) {
        try {
            if (!this.isValidInstance()) {
                this.logger.log(LOG_LEVEL.ERROR, sprintf(LOG_MESSAGES.INVALID_OBJECT, MODULE_NAME$d, 'isFeatureEnabled'));
                return false;
            }
            ...
            if (featureEnabled === true) {
                this.logger.log(LOG_LEVEL.INFO, sprintf(LOG_MESSAGES.FEATURE_ENABLED_FOR_USER, MODULE_NAME$d, featureKey, userId));
            }
            else {
                this.logger.log(LOG_LEVEL.INFO, sprintf(LOG_MESSAGES.FEATURE_NOT_ENABLED_FOR_USER, MODULE_NAME$d, featureKey, userId));
    ...
    Optimizely.prototype.getEnabledFeatures = function (userId, attributes) {
        ...
        try {
            ...
            if (!this.isValidInstance()) {
                this.logger.log(LOG_LEVEL.ERROR, sprintf(LOG_MESSAGES.INVALID_OBJECT, MODULE_NAME$d, 'getEnabledFeatures'));
    ...
    Optimizely.prototype.getFeatureVariable = function (featureKey, variableKey, userId, attributes) {
        try {
            if (!this.isValidInstance()) {
                this.logger.log(LOG_LEVEL.ERROR, sprintf(LOG_MESSAGES.INVALID_OBJECT, MODULE_NAME$d, 'getFeatureVariable'));
                return null;
            }
    ...
    Optimizely.prototype.getFeatureVariableForType = function (featureKey, variableKey, variableType, userId, attributes) {
        ...
        if (variableType && variable.type !== variableType) {
            this.logger.log(LOG_LEVEL.WARNING, sprintf(LOG_MESSAGES.VARIABLE_REQUESTED_WITH_WRONG_TYPE, MODULE_NAME$d, variableType, variable.type));
            return null;
        }
    ...
    Optimizely.prototype.getFeatureVariableValueFromVariation = function (featureKey, featureEnabled, variation, variable, userId) {
        ...
        if (variation !== null) {
            ...
            if (value !== null) {
                if (featureEnabled) {
                    variableValue = value;
                    this.logger.log(LOG_LEVEL.INFO, sprintf(LOG_MESSAGES.USER_RECEIVED_VARIABLE_VALUE, MODULE_NAME$d, variableValue, variable.key, featureKey));
                }
                else {
                    this.logger.log(LOG_LEVEL.INFO, sprintf(LOG_MESSAGES.FEATURE_NOT_ENABLED_RETURN_DEFAULT_VARIABLE_VALUE, MODULE_NAME$d, featureKey, userId, variableValue));
                }
            }
            else {
                this.logger.log(LOG_LEVEL.INFO, sprintf(LOG_MESSAGES.VARIABLE_NOT_USED_RETURN_DEFAULT_VARIABLE_VALUE, MODULE_NAME$d, variable.key, variation.key));
            }
        }
        else {
            this.logger.log(LOG_LEVEL.INFO, sprintf(LOG_MESSAGES.USER_RECEIVED_DEFAULT_VARIABLE_VALUE, MODULE_NAME$d, userId, variable.key, featureKey));
        }
    ...
    Optimizely.prototype.getFeatureVariableBoolean = function (featureKey, variableKey, userId, attributes) {
        try {
            if (!this.isValidInstance()) {
                this.logger.log(LOG_LEVEL.ERROR, sprintf(LOG_MESSAGES.INVALID_OBJECT, MODULE_NAME$d, 'getFeatureVariableBoolean'));
                return null;
            }
    ...
    Optimizely.prototype.getFeatureVariableDouble = function (featureKey, variableKey, userId, attributes) {
        try {
            if (!this.isValidInstance()) {
                this.logger.log(LOG_LEVEL.ERROR, sprintf(LOG_MESSAGES.INVALID_OBJECT, MODULE_NAME$d, 'getFeatureVariableDouble'));
                return null;
            }
    ...
    Optimizely.prototype.getFeatureVariableInteger = function (featureKey, variableKey, userId, attributes) {
        try {
            if (!this.isValidInstance()) {
                this.logger.log(LOG_LEVEL.ERROR, sprintf(LOG_MESSAGES.INVALID_OBJECT, MODULE_NAME$d, 'getFeatureVariableInteger'));
                return null;
            }
    ...
    Optimizely.prototype.getFeatureVariableString = function (featureKey, variableKey, userId, attributes) {
        try {
            if (!this.isValidInstance()) {
                this.logger.log(LOG_LEVEL.ERROR, sprintf(LOG_MESSAGES.INVALID_OBJECT, MODULE_NAME$d, 'getFeatureVariableString'));
                return null;
            }
    ...
    Optimizely.prototype.getFeatureVariableJSON = function (featureKey, variableKey, userId, attributes) {
        try {
            if (!this.isValidInstance()) {
                this.logger.log(LOG_LEVEL.ERROR, sprintf(LOG_MESSAGES.INVALID_OBJECT, MODULE_NAME$d, 'getFeatureVariableJSON'));
                return null;
            }
    ...
    Optimizely.prototype.getAllFeatureVariables = function (featureKey, userId, attributes) {
        ...
        try {
            if (!this.isValidInstance()) {
                this.logger.log(LOG_LEVEL.ERROR, sprintf(LOG_MESSAGES.INVALID_OBJECT, MODULE_NAME$d, 'getAllFeatureVariables'));
                return null;
            }
            if (!this.validateInputs({ feature_key: featureKey, user_id: userId }, attributes)) {
                return null;
    ...
    Optimizely.prototype.decide = function (user, key, options) {
        ...
        if (!this.isValidInstance() || !configObj) {
            ...
            this.logger.log(LOG_LEVEL.INFO, sprintf(LOG_MESSAGES.INVALID_OBJECT, MODULE_NAME$d, 'decide'));
        ...
        if (!feature) {
            ...
            this.logger.log(LOG_LEVEL.ERROR, sprintf(ERROR_MESSAGES.FEATURE_NOT_IN_DATAFILE, MODULE_NAME$d, key));
            return newErrorDecision(key, user, reasons);
        }
        ...
        if (flagEnabled === true) {
            this.logger.log(LOG_LEVEL.INFO, sprintf(LOG_MESSAGES.FEATURE_ENABLED_FOR_USER, MODULE_NAME$d, key, userId));
        }
        else {
            this.logger.log(LOG_LEVEL.INFO, sprintf(LOG_MESSAGES.FEATURE_NOT_ENABLED_FOR_USER, MODULE_NAME$d, key, userId));
        }
        var variablesMap = {};
    ...
    Optimizely.prototype.getAllDecideOptions = function (options) {
        ...
        if (!Array.isArray(options)) {
            this.logger.log(LOG_LEVEL.DEBUG, sprintf(LOG_MESSAGES.INVALID_DECIDE_OPTIONS, MODULE_NAME$d));
        }
        else {
    ...
    Optimizely.prototype.decideForKeys = function (user, keys, options) {
        ...
        var decisionMap = {};
        if (!this.isValidInstance()) {
            this.logger.log(LOG_LEVEL.ERROR, sprintf(LOG_MESSAGES.INVALID_OBJECT, MODULE_NAME$d, 'decideForKeys'));
            return decisionMap;
        }
        if (keys.length === 0) {
            return decisionMap;
    ...
    Optimizely.prototype.decideAll = function (user, options) {
        ...
        var decisionMap = {};
        if (!this.isValidInstance() || !configObj) {
            this.logger.log(LOG_LEVEL.ERROR, sprintf(LOG_MESSAGES.INVALID_OBJECT, MODULE_NAME$d, 'decideAll'));
            return decisionMap;
        }
...
var createInstance = function (config) {
    try {
        ...
        if (!eventProcessorConfigValidator.validateEventBatchSize(config.eventBatchSize)) {
            logger$5.warn('Invalid eventBatchSize %s, defaulting to %s', config.eventBatchSize, DEFAULT_EVENT_BATCH_SIZE);
            eventBatchSize = DEFAULT_EVENT_BATCH_SIZE;
        }
        if (!eventProcessorConfigValidator.validateEventFlushInterval(config.eventFlushInterval)) {
            logger$5.warn('Invalid eventFlushInterval %s, defaulting to %s', config.eventFlushInterval, DEFAULT_EVENT_FLUSH_INTERVAL);
            eventFlushInterval = DEFAULT_EVENT_FLUSH_INTERVAL;
        }
        var optimizelyOptions = __assign(__assign({ clientEngine: JAVASCRIPT_CLIENT_ENGINE, eventDispatcher: eventDispatcher }, config), { eventBatchSize: eventBatchSize, eventFlushInterval: eventFlushInterval, logger: logger$5, errorHandler: getErrorHandler() });
...
export { OptimizelyDecideOption, __internalResetRetryState, createInstance, enums, defaultErrorHandler as errorHandler, defaultEventDispatcher as eventDispatcher, loggerPlugin as logging };
